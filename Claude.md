...
> Below is a draft of the claude.md file that we will give to the agent together with a paper.

## Goal

The goal is to develop a multi-step / multi-agent system that automatically runs a replication of a social science paper.

We divide the work into two steps: paper collection and the creation of the evaluation "scaffolds". Then, the creation and execution of an agentic pipeline to replicate the papers.

## Paper Collection

### Step 0: Collect papers and data

Given a list of papers, find their replication data and store the PDFs and data in separate folders

### Step 1 / Agent 1: Extractor of main results and methods from papers

This agent should receive a paper in pdf as input.

The agent should return a structured document which defines inputs for the replication steps
...
It should return a classification for each plot and table that tells whether the agent was able to replicate the result from the original paper as well as an overall classification of the reproduction effort.
...
- **B**: Replicated the results with the same direction of effects and trends but small discrepancies in the numerical outcomes.
- **C**: Replicated the results with the same direction of effects and trends but large discrepancies in the numerical outcomes.
- **D**: Did not replicate the results, the replicated effects or trends relevantly differ from the original results. The effects might be non-detectable or even pointing in opposite directions
...
- a description of the discrepancies between the replicated and original results
- an analysis why these discrepancies occured
- a classification of the reasons for why this error occurred is identifiable from the documents or should not occur (e.g., because a different code was used to generate the results than in the replication package). This classification should be binary
- a judgment of given the papers intend and research question whether these discrepancies are rather the fault of the replicator (e.g., because they used wrong assumptions or misspecified a method) or by the orignal paper (e.g., by using implausible data cleaning steps, wrong assumptions)

## Methods

- The code for this multi-agent system should use the opencode framework so that underlying models can easily switched
